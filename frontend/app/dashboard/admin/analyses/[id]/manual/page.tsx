...
export default function AdminManualAnalysisPage() {
  ...
  const [analysis, setAnalysis] = useState<AdminAnalysisDetailResponse | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    const payload: AdminUpdateAnalysisRequest = {};
    ...
    if (Object.keys(payload).length === 0) {
      ...
      router.push(`/dashboard/admin/analyses/${analysisId}/summary`);
      return;
    }

    try {
      setSaving(true);
      const updated = await apiClient.updateAdminAnalysis(analysisId, payload);

      const cuotaDisponible =
        updated.valor_cuota_con_subsidio ??
        updated.valor_cuota_con_seguros ??
        updated.valor_cuota_sin_seguros;

      const hasAllRequired =
        updated.valor_prestado_inicial !== null &&
        updated.valor_prestado_inicial !== undefined &&
        updated.saldo_capital_pesos !== null &&
        updated.saldo_capital_pesos !== undefined &&
        cuotaDisponible !== null &&
        cuotaDisponible !== undefined &&
        updated.cuotas_pendientes !== null &&
        updated.cuotas_pendientes !== undefined &&
        updated.tasa_interes_cobrada_ea !== null &&
        updated.tasa_interes_cobrada_ea !== undefined;

      if (!hasAllRequired) {
        toast.error('Aún faltan datos mínimos para proyectar. Completa los campos pendientes.');
      ...
      <Card className="border-t-4 border-blue-500">
        <div className="p-6 space-y-2">
          ...
          <p className="text-sm text-gray-600">
            El PDF quedó guardado. Completa los datos mínimos faltantes para generar proyecciones del cliente.
          </p>
        </div>
      </Card>
      ...
      {missingFields.length > 0 ? (
        <Card>
          ...
          <div className="px-6 pb-6 flex justify-end gap-3">
            <Link href={`/dashboard/admin/analyses/${analysisId}`}>
